import { Component, Input, ViewChild, ElementRef, Output, OnInit, SimpleChange, Renderer, EventEmitter } from '@angular/core';
...
export class ThsMapComponent implements OnInit {
  ...
  // tile信息设置
  groupLayers = [];
  showAllImg = false; // 显示全部的按钮
  constructor(private mapService: ThsMapService, private renderer: Renderer) { }
    ...
    return new Promise((resolve, reject) => {
      loadModules(['esri/renderers/jsonUtils', 'esri/layers/WebTiledLayer', 'esri/layers/FeatureLayer',
        ...
        'esri/layers/LabelClass']).then(([jsonUtil, WebTiledLayer, FeatureLayer, ArcGISDynamicMapServiceLayer,
          ...
          const tiledInfoObj = this.getTiledInfoObj(layerType);
          let tileInfo = null;
          if (tiledInfoObj !== null) {
            tileInfo = new TileInfo(tiledInfoObj);
          }
          let urlTemplate = '';
          // layerType = 'autonavigaode';
          ...
          } else if (layerType.indexOf('autonavi') > -1) { // 高德底图
  ...
  showHome() {
    ...
    // this.mapService.setCenterAndZoom(this.map, 106.22424, 34.495428, 3);
  }
  /**
   ...
   */
  // location() {
  //   this.mapService.getLocation(this.map, this.configService.location.longitude, this.configService.location.latitude, 10);
  // }
